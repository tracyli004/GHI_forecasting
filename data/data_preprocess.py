...
data_df["time_idx"] = data_df["Year"] * 12 + data_df["Month"]
...
for col in log_cols:
    data_df[f"log_{col}"] = np.log(data_df[col] + 1e-8)
...
data_df["weather"] = data_df[weather_cols].apply(lambda x: x.idxmax(axis=1).replace("Daylight Weather_", ""), axis=1)
...
# Save processed data
data_df.to_csv("data/train_x_preprocessed.csv", index=False)
